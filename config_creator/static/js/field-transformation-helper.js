 ...
 */
function switchFunctionType (selectedType) { // eslint-disable-line no-unused-vars
  if (!selectedType) {
    return
  }
...
function getFunction (functionName, functionType) {
  for (let i = 0; functionObject[`${functionType}`].length; i++) {
    const func = functionObject[`${functionType}`][i]
 ...
 *   the function object.
 */
function displayFunction (selectedFunction) { // eslint-disable-line no-unused-vars
  if (!selectedFunction) {
    return
  }
...
function populateFunctionSelect (targetId, content) {
  ...
  for (let i = 0; i < content.length; i++) {
    const opt = createElement('option', content[i].name) // eslint-disable-line no-undef
 ...
 */
function functionDescriptionReSize () { // eslint-disable-line no-unused-vars
  this.style.height = 0
 ...
 */
function copySyntax () {
  textToClipboard(getSyntax(), 'id_field_transformation_modal') // eslint-disable-line no-undef
}
 ...
 */
function saveTransformation () { // eslint-disable-line no-unused-vars
  const splitString = document.getElementById('id_modal_field_transformation').value.split(' ')
  let transformationString = ''

  for (let i = 0; i < splitString.length; i++) {
    const match = splitString[i].match(/'.*'|".*"/)
    if (match) {
